...
#     if(tally > 0){
#       out[j] <- floor(tally) ; tally <- tally-floor(tally);
#     } 
#   }
#   return(out)
# }
...
zAdjust <- function(obs=rawcarcasses,cc=ccmatrix){
  x1 <- sum(obs[,2])
  x2 <- sum(obs[,3])
  x3 <- sum(obs[,4])
  x4 <- sum(obs[,5])
...
x <- read.csv("carcasses.2004.2023.csv")  
...
print(cbind(carcassrawsections, carcassAdjustsections))
...
for(year in allyears){ 
  ...
  holdDat <- cbind.data.frame(Year=year,X1=NA,X2=NA,X3=NA,X4=NA,Totals=NA)
    ...
    for(colu in 2:ncol(use)){
      ...
      if(scalar > 0){
        newuse[[colu]] <- zExpand(use[[colu]] ,scalar) 
      } else {
...
for(fig in c("pdf", "" )){   # """png" ){   #  
  ...
  x[x<0] = 0
  spacespace <- 0.2
  barplot(t(x[,c(5:2)]),border=NA,col=colos,ylim=c(-5,2000),space=spacespace,ylab="Counts")
  ...
  text(4,1800,"Aerial Survey Distributions",adj=0)
  x <- carcassAdjustsections
  x[x<0] = 0
  barplot(t(x[,c(5:2)]),border=NA,col=colos,ylim=c(-5,3000),space=spacespace,ylab="Counts")
  legend("topright",bty="n",fill=rev(colos),legend=c("Reach 1","Reach 2","Reach 3","Reach 4"))
  axis(1,labels=allyears,at=(allyears-2003.5)*(1  + spacespace))
  text(4,2800,"Estimated Spatial Distributions",adj=0)
  if(fig=="pdf" | fig == "png") dev.off()
}
par(mgp=c(3,1,0))

#==== Draw figure for  ms. Four Panel Figure====
for(fig in c("pdf")){  #   c("","png","pdf")){  #   
  ...
  mtext("(a) Redds from Aerial Surveys ",line=0.5,adj=.4,cex=cexpanel)
  ...
  mtext("(b) Estimated Redds from Carcass Surveys",line=0.5,adj=.1,cex=cexpanel)
